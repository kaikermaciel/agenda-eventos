...
void inserir_conteudo_lse (t_lse* lse, void* carga){
    ...
    if(lse){
        if(cam == NULL || lse->comparar(cam->carga_util, carga) > 0){inserir_lse(lse, carga);}
        else{
...
void* imprimir_proximo_lse(t_lse *lse){
    if(lse){
        t_elemento_lse *cam = lse->inicio;
        lse->imprimir(cam->carga_util);
    }
...
void* busca_lista(t_lse* lse, void* carga){
    t_elemento_lse *cam = lse->inicio;
    if(lse){
        if(cam){
            while(cam){
...
void* remove_busca_lse(t_lse* lse, void* carga) {
    t_elemento_lse *cam, *ant;
    void* dado = NULL;
    if(lse && lse->inicio){
        cam = lse->inicio;
        if(lse->comparar(cam->carga_util, carga) == 0){
            ...
            while(cam->prox || lse->comparar(cam->carga_util, carga) != 0){
                ant = cam;
                cam = cam->prox;                
            }
            if(!cam->prox){
